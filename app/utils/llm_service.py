...
class LLMService:
    client: AzureOpenAI
    deployment_name: str # Added type hint
    ...
    def __init__(self, endpoint_url: HttpUrl, api_key: str):
        ...
        # --- CORRECTED DEPLOYMENT NAME LOGIC ---
        try:
            # Example URL format: https://<res>.openai.azure.com/openai/deployments/<dep_name>/...
            # Access the .path attribute which is already a string
    ...
    def init_singleton(endpoint_url: HttpUrl, api_key: str):
        ...
        if llm_service is None:
            ...
            try: # Add try-except around initialization
                llm_service = LLMService(endpoint_url, api_key)
                logger.info("LLMService singleton initialized successfully.")
            except Exception as e:
                 logger.error(f"Failed to initialize LLMService singleton: {e}", exc_info=True)
                 raise RuntimeError("LLMService singleton initialization failed.") from e
        else:
