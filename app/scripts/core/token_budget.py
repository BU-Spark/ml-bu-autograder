...
MODEL_PRICING: dict[str, dict[str, float]] = {
    ...
    # Unversioned alias — same rates as the pinned version below; kept so callers that
    # omit the date suffix still get a cost estimate.
    "claude-haiku-4-5": {
        "input": 1.00,
        "output": 5.00,
        "cache_creation_input": 1.25,
    ...
    # FIXME: verify pricing — rates below may be a copy-paste from gemini-2.0-flash.
    # TODO: Gemini 2.5 Flash uses tiered pricing; update with correct standard-tier rates.
    "gemini-2.5-flash": {
        "input": 0.15,
        "output": 0.60,
    },
...
class TokenTracker:
    ...
    Not thread-safe; instantiate one tracker per grading run.
    """
    ...
    def record(self, call_type: str, model: str, usage: dict[str, int]) -> None:
        ...
        self._calls.append({
            ...
            "grading_call_cost_usd": _estimate_cost(model, usage),
        })
        ...
        for call in self._calls:
            ...
            if ct not in buckets:
                buckets[ct] = {
                    ...
                    "grading_call_cost_usd": 0.0,
                }
            b = buckets[ct]
            b["count"] += 1
            for key in ("input_tokens", "output_tokens", "cache_creation_input_tokens",
                        "cache_read_input_tokens", "grading_call_cost_usd"):
                b[key] += call[key]
        # Round once per bucket after all accumulation to avoid per-step float drift.
        for b in buckets.values():
            b["grading_call_cost_usd"] = round(b["grading_call_cost_usd"], 6)
        return buckets
    ...
    def total_summary(self) -> dict[str, Any]:
        ...
        total_cost = round(sum(c["grading_call_cost_usd"] for c in self._calls), 6)
        return {
            ...
            "total_grading_call_cost_usd": total_cost,
        }
    ...
    def human_summary(self) -> str:
        ...
        lines = [
            ...
            f"  Grading call cost: ${t['total_grading_call_cost_usd']:.4f} USD",
            "─────────────────────────────────────────────",
        ]
        by_type = self.summary_by_type()
        if len(by_type) > 1:
            for ct, b in by_type.items():
                lines.append(f"  [{ct}] {b['count']} call(s)  in={b['input_tokens']:,}  out={b['output_tokens']:,}  ${b['grading_call_cost_usd']:.4f}")
        return "\n".join(lines)
