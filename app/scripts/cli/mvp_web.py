...
DEFAULT_LECTURE_CHUNKS = (
    ...
    / "outputs"
    / "final_phase1"
    / "lecture_chunks_hybrid.jsonl"
)
DEFAULT_RUBRIC_ROOT = Path(
    os.getenv("AUTO_GRADER_RUBRIC_DIR", str(PROJECT_ROOT / "assignments" / "rubrics"))
)
...
""",
    ...
    with st.sidebar:
        ...
        with st.expander("Optional Preselected Files"):
            selected_assignment = st.selectbox("Assignment file", candidate_labels, index=0)
            selected_rubric = st.selectbox("Rubric file", candidate_labels, index=0)
    ...
    with c1:
        ...
        student_upload = st.file_uploader(
            ...
            help="Supported formats: PDF, PPTX, and XLSX.",
        )
