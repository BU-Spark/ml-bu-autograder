...
import Link from 'next/link'; // Keep for potential links
import {
  ...
  Link as MuiLink // Keep MUI Link if used
} from '@mui/material';
...
export default function RubricManagementPage() {
  ...
  const [isSaving, setIsSaving] = useState(false); // Specific state for save action
  ...
  // *** CORRECTED: Added setActionLoading ***
  const [actionLoading, setActionLoading] = useState(false); // Generic loading for OTHER actions (AI, criteria)
  ...
  // --- DATA FETCHING ---
  const createEmptyRubric = useCallback((assignment) => { if (!assignment || !Array.isArray(assignment.questions) || !semester || !courseId) { console.error("Cannot create empty rubric: Invalid assignment object or missing context."); return null; } const sortedQuestions = [...assignment.questions].sort((a,b) => (a.question_index ?? Infinity) - (b.question_index ?? Infinity)); const subRubrics = sortedQuestions.map(question => ({ question_index: question.question_index, max_points: 10, instructor_guideline: '', leniency: null, grading_criteria: [], })); return { semester, course_id: courseId, assignment_id: String(assignment.assignment_id), grading_flags: [], overall_instructor_guidelines: '', sub_rubrics: subRubrics, leniency: 3 }; }, [semester, courseId]);
  const fetchRubric = useCallback(async (assignment) => {
      ...
      try {
          ...
          const validRubric = { ...createEmptyRubric(assignment), ...rubricData, assignment_id: assignmentIdToFetch };
          const currentQuestionIndices = new Set(assignment.questions?.map(q => q.question_index) ?? []);
          validRubric.sub_rubrics = (Array.isArray(validRubric.sub_rubrics) ? validRubric.sub_rubrics : []).filter(sr => currentQuestionIndices.has(sr.question_index)).map(sr => ({ ...sr, grading_criteria: Array.isArray(sr?.grading_criteria) ? sr.grading_criteria : [] }));
          assignment.questions?.forEach(q => { if (!validRubric.sub_rubrics.some(sr => sr.question_index === q.question_index)) { validRubric.sub_rubrics.push({ question_index: q.question_index, max_points: 10, leniency: null, instructor_guideline: '', grading_criteria: [] }); } });
          validRubric.sub_rubrics.sort((a, b) => (a.question_index ?? Infinity) - (b.question_index ?? Infinity));
          setRubric(validRubric);
          if (validRubric.sub_rubrics.length > 0) { setCurrentQuestionIndex(validRubric.sub_rubrics[0].question_index); setTabValue(1); } else { setCurrentQuestionIndex(null); setTabValue(0); }
      } catch (err) {
          console.error(`Error fetching rubric for ${assignmentIdToFetch}:`, err);
          if (err.response && err.response.status === 404) {
              console.log("Rubric not found (404), creating empty structure.");
              const emptyRubric = createEmptyRubric(assignment);
              setRubric(emptyRubric);
              if (emptyRubric?.sub_rubrics?.length > 0) { setCurrentQuestionIndex(emptyRubric.sub_rubrics[0].question_index); setTabValue(1); } else { setCurrentQuestionIndex(null); setTabValue(0); }
      ...
      try {
        ...
        if (isMounted) {
            if (Array.isArray(assignmentsData)) {
               const validAssignments = (assignmentsData || []).filter(a => a && typeof a === 'object' && typeof a.assignment_id !== 'undefined').map(a => ({...a, questions: (Array.isArray(a?.questions) ? a.questions : []).sort((qa, qb) => (qa.question_index ?? Infinity) - (qb.question_index ?? Infinity))})); // Sort questions here
               ...
               else { setLoadingRubric(false); } // No assignment, no rubric to load
  ...
  }, [router.isReady, courseId, semester, assignmentIdFromUrl, fetchRubric, formatApiError, showAlert, router]); // Added missing dependencies
  ...
  const applyAIRubricSuggestions = useCallback(() => { if (!aiRubricSuggestion || !selectedAssignment) return; if (aiRubricSuggestion.semester === semester && aiRubricSuggestion.course_id === courseId && String(aiRubricSuggestion.assignment_id) === String(selectedAssignment.assignment_id)) { const alignedRubric = { ...createEmptyRubric(selectedAssignment), ...aiRubricSuggestion }; alignedRubric.assignment_id = String(alignedRubric.assignment_id); const currentQuestionIndices = new Set(selectedAssignment.questions?.map(q => q.question_index) ?? []); alignedRubric.sub_rubrics = (alignedRubric.sub_rubrics || []).filter(sr => currentQuestionIndices.has(sr.question_index)).map(sr => ({...sr, grading_criteria: Array.isArray(sr.grading_criteria) ? sr.grading_criteria : [] })); selectedAssignment.questions?.forEach(q => { if (!alignedRubric.sub_rubrics.some(sr => sr.question_index === q.question_index)) { alignedRubric.sub_rubrics.push({ question_index: q.question_index, max_points: 10, leniency: null, instructor_guideline: '', grading_criteria: [] }); } }); alignedRubric.sub_rubrics.sort((a, b) => (a.question_index ?? Infinity) - (b.question_index ?? Infinity)); setRubric(alignedRubric); setAiRubricSuggestion(null); setEditMode(true); showAlert('AI suggestions applied to the editor.', 'info'); } else { showAlert('Cannot apply: Identifiers mismatch.', 'error'); } }, [aiRubricSuggestion, semester, courseId, selectedAssignment, showAlert, createEmptyRubric]);
  const saveRubric = useCallback(async () => { if (!rubric || !courseId || !semester || !selectedAssignment?.assignment_id) return showAlert("Cannot save: Missing context.", "warning"); const validationIssues = rubric.sub_rubrics?.filter(sr => calculateTotalPoints(sr) !== parseFloat(sr.max_points || 0)); if (validationIssues?.length > 0) { const issueDetails = validationIssues.map(sr => `Q${(sr.question_index ?? '?') + 1}`).join(', '); showAlert(`Cannot save: Points mismatch for ${issueDetails}. Please correct before saving.`, 'error'); return; } setIsSaving(true); try { const payload = { ...rubric, semester: semester, course_id: courseId, assignment_id: selectedAssignment.assignment_id }; await rubricService.createOrUpdateRubric(payload); setEditMode(false); showAlert('Rubric saved successfully.'); if(selectedAssignment) { await fetchRubric(selectedAssignment); } } catch (err) { console.error('Error saving rubric:', err); showAlert(formatApiError(err, 'Failed to save rubric.'), 'error'); } finally { setIsSaving(false); } }, [rubric, courseId, semester, selectedAssignment, fetchRubric, showAlert, formatApiError]);
  ...
  const renderAssignmentSelection = () => {
    if (loadingAssignments) return <CardSkeleton height={100} sx={{mb: 4}} />;
    if (error) return <Alert severity="error" sx={{ mb: 3 }}>{error}</Alert>;
    if (!Array.isArray(assignments) || !assignments.length) return <NoItemsBox><AssignmentIcon sx={{ fontSize: 60, color: 'text.secondary', mb: 2 }} /><Typography variant="h6">No assignments found for {courseId}/{semester}.</Typography><Typography color="text.secondary">Create assignments first.</Typography></NoItemsBox>;

    return (
      <FormControl fullWidth variant="outlined" sx={{ mb: 3 }}>
        <InputLabel id="assignment-select-label">Select Assignment</InputLabel>
        <Select labelId="assignment-select-label" value={selectedAssignment?.assignment_id ?? ''} onChange={(e) => { const assignment = assignments.find((a) => String(a.assignment_id) === e.target.value); if (assignment) handleSelectAssignment(assignment); }} label="Select Assignment" disabled={loadingAssignments || loadingRubric || isSaving || actionLoading} >
  ...
  const renderRubricContent = () => {
    if (loadingRubric) return <Box sx={{ display: 'flex', justifyContent: 'center', p: 5 }}><CircularProgress /> <Typography sx={{ ml: 2 }}>Loading rubric...</Typography></Box>;
    if (!selectedAssignment && !loadingAssignments && assignments.length > 0) return <Alert severity="info" sx={{ mt: 2 }}>Please select an assignment above to view or edit its rubric.</Alert>;
    if (detailsError && !rubric) return <Alert severity="error" sx={{ mt: 2 }}>{detailsError}</Alert>; // Show rubric-specific error
    if (!rubric && selectedAssignment && !loadingRubric) return <Alert severity="warning" sx={{ mt: 2 }}>Rubric data unavailable or failed to create default. Try re-selecting the assignment.</Alert>;
    if (!selectedAssignment || !rubric) return null; // Don't render if no assignment selected or rubric failed completely

    const currentSubRubric = rubric.sub_rubrics?.find(sr => sr.question_index === currentQuestionIndex);

    return (
      <>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, flexWrap: 'wrap', gap: 1 }}> <Typography variant="h5">{`Rubric: ${selectedAssignment.assignment_id}`}</Typography> <Box> {editMode ? (<> <Button variant="contained" color="primary" size="small" startIcon={isSaving ? <CircularProgress size={20} color="inherit" /> : <SaveIcon />} onClick={saveRubric} sx={{ mr: 1 }} disabled={isSaving || loadingAI}> Save Rubric </Button> <Button variant="outlined" size="small" onClick={() => { setEditMode(false); fetchRubric(selectedAssignment); }} disabled={isSaving}> Cancel Edit </Button> </>) : (<> <Button variant="outlined" size="small" startIcon={<LightbulbIcon />} onClick={() => setAiInstructionsDialogOpen(true)} sx={{ mr: 1 }} disabled={loadingAI || isSaving}>{loadingAI ? 'Generating...' : 'AI Suggestions'}</Button> <Button variant="contained" size="small" startIcon={<EditIcon />} onClick={() => setEditMode(true)} disabled={isSaving}> Edit Rubric </Button> </>)} </Box> </Box>
  ...
  return (
    <Box sx={{ p: { xs: 1, sm: 2, md: 3 } }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 1 }}>
        <IconButton title="Back to Course Overview" aria-label="back" onClick={() => (courseId && semester) && router.push(`/course/${courseId}?semester=${semester}`)} disabled={!courseId || !semester || actionLoading || loadingAssignments || loadingRubric || isSaving} sx={{ mr: 1 }}> <ArrowBackIcon /> </IconButton>
        <Typography variant="h4" component="h1" sx={{ flexGrow: 1, mr: 1 }}>Rubric Management</Typography>
      </Box>
      {/* Assignment Selector */}
      {renderAssignmentSelection()}
      <Divider sx={{ my: 3 }} />
      {/* Rubric Content (conditionally rendered) */}
      {selectedAssignment && renderRubricContent()}

      {/* --- DIALOGS --- */}
      <Dialog open={criteriaDialogOpen} onClose={() => !actionLoading && setCriteriaDialogOpen(false)} maxWidth="sm" fullWidth>
      ...
      <Dialog open={aiInstructionsDialogOpen} onClose={() => !loadingAI && setAiInstructionsDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent><Typography variant="body2" paragraph>Provide specific instructions to guide the AI, or leave blank for general suggestions based on assignment content.</Typography><TextField label="Instructions for AI (Optional)" fullWidth multiline rows={4} value={aiInstructions} onChange={(e) => setAiInstructions(e.target.value)} margin="normal" placeholder="e.g., Focus on accuracy, deduct for syntax errors, give partial credit for..." disabled={loadingAI} /></DialogContent>
        <DialogActions><Button onClick={() => setAiInstructionsDialogOpen(false)} disabled={loadingAI}>Cancel</Button><Button onClick={getAIRubricSuggestions} variant="contained" color="primary" disabled={loadingAI || !selectedAssignment} startIcon={loadingAI ? <CircularProgress size={20} color="inherit"/> : <LightbulbIcon />}>{loadingAI ? 'Generating...' : 'Get Suggestions'}</Button></DialogActions>
      </Dialog>
