...
def flatten_system_blocks(blocks: list[dict[str, Any]]) -> str:
    ...
    Only blocks with ``type == "text"`` are included. Any other block type raises
    ValueError so callers are not silently missing content.
    """
    parts = []
    for b in blocks:
        btype = b.get("type")
        if btype == "text":
            ...
            raise ValueError(
                f"flatten_system_blocks: unsupported block type '{btype}' for non-Anthropic provider"
            )
    return "\n\n".join(parts)
...
def build_system_blocks(
    *,
    ...
    When ``enable_cache=True`` (Anthropic only), a single ``cache_control`` breakpoint
    is placed on block2 — the outermost stable boundary. This covers both blocks in one
    cache entry. Block1 intentionally has no breakpoint to avoid a redundant intermediate
    cache write.
    """
    # --- Block 1: system prompt (no cache_control — covered by block2's breakpoint) ---
    block1: dict[str, Any] = {
        "type": "text",
        "text": SYSTEM_PROMPT,
    }

    # --- Block 2: assignment + rubric context ---
    ...
    # TODO: few-shot examples will be appended here once teammate's implementation is ready
    context_text = (
...
def build_user_message(
    ...
    # Keep the first 70% and last 30% so both intro and conclusion are visible to the grader.
...
def read_text_file(path: Path | None) -> str:
    ...
    # PDF
    if suffix == ".pdf":
        # Use PyMuPDF (fitz) — already installed as part of the extraction pipeline
        try:
            import fitz  # PyMuPDF
